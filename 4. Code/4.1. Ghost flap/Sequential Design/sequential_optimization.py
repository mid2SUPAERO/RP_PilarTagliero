...
import numpy as np
from functions import airfoil, V_flutter, value2norm, norm2value, objective_m, constraints_m, objective_k, constraints_k, mass_velocities_m, plot_history_m, mass_velocities_k, plot_history_k, save_results
...
#input_fb = np.array([1, 1, 1, 1])
#K = np.array([-0.05, 0.1, -0.001, 0.006])
#wing1 = AE_model_CL(aero_coef, rho_air, ea_b, c, s, m, I_alpha, x_alpha, w_alpha, w_h, output_choice,K, input_fb)
...
K = np.array([0, 0, 0, 0]) # No controller


#%% OPTIMIZATION MASS

## Initialization
DV_init = th #Design variables

lb_DV = np.array([geom_prop['c']/20, geom_prop['c']/20, geom_prop['h']/20, geom_prop['h']/20, 0])
ub_DV = np.array([geom_prop['c']/10, geom_prop['c']/10, geom_prop['h']/10, geom_prop['h']/10, geom_prop['c']-2*geom_prop['c']/10])
...
# Constraints
myConstr = ({'type': 'ineq', 'fun': constraints_m, 'args':(rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, K, EF)})
...
# Optimization - Mass
opt = minimize(objective_m, 
               x_init, 
               args=(rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, K, EF,),
               method=Met,
               jac = '2-point',
               bounds=Bnd,
...
k = np.arange(0, opt.nit+1, 1)
    
xk_value = []
    
for it in k:
    x = norm2value(xk_m[it], lb_DV, ub_DV)
    xk_value.append(x)
 
history = {'th1':[], 'th2':[], 'th3':[], 'th4':[], 'th5':[], 'm':[], 'Vf_OL':[], 'Vf_CL':[], 'f_obj':[], 'constraint': []}
...
## Mass and flutter velocities for each xk
    [m, Vf_OL, Vf_CL] = mass_velocities_m(it, aero_coef, history, rho_air, geom_prop, mat_prop, V, output_choice, EF, K)
...
geom_prop['th'] = xk_value[-1]

## Initialization
DV_init = K #Design variables

lb_DV = np.array([-Kh, -Ka, -Khd, -Kad])
ub_DV = np.array([Kh, Ka, Khd, Kad])
...
# Constraints
myConstr = ({'type': 'ineq', 'fun': constraints_k, 'args':(rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, EF)})
...
# Save results here
eps = 5e-2
case = 'controller_optim'
os.mkdir('Results/'+str(case))

time_start = perf_counter()

# Optimization - Controller
opt = minimize(objective_k, 
               x_init, 
               args=(rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, EF),
               method=Met,
               jac = '2-point',
               bounds=Bnd,
               constraints=myConstr,
...
for it in k:
    x = norm2value(xk_k[it], lb_DV, ub_DV)
    xk_value.append(x)
 
history = {'Ka':[], 'Kad':[], 'Kh':[], 'Khd':[], 'm':[], 'Vf_OL':[], 'Vf_CL':[], 'f_obj':[], 'constraint': [], 'EF':[]}

for it in k:
            
    history['Kh'].append(xk_value[it][0])
    history['Ka'].append(xk_value[it][1])
    history['Khd'].append(xk_value[it][2])
    history['Kad'].append(xk_value[it][3])

        
## Mass and flutter velocities for each xk
    [m, Vf_OL, Vf_CL] = mass_velocities_k(it, aero_coef, history, rho_air, geom_prop, mat_prop, V, output_choice, EF)
