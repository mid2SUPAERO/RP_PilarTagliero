...
def objective_m (x, *args):
    rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, K, EF = args

# OBJECTIVE FUNCTION FOR THE MASS OPTIMIZATION
...
def constraints_m (x, rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, K, EF):

# CONSTRAINT FOR THE MASS OPTIMIZATION    
    ...
    # Compute and return flutter constraint
    return Vf_CL/Ref[1]-1


###############################################################################

def objective_k (x, *args):
    rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, EF = args

# OBJECTIVE FUNCTION FOR THE GAINS OPTIMIZATION
...
def constraints_k (x, rho_air, geom_prop, mat_prop, Ref, lb_DV, ub_DV, V, output_choice, aero_coef, EF):

# CONSTRAINT FOR THE GAINS OPTIMIZATION
...
def mass_velocities_m (j, aero_coef, history, rho_air, geom_prop, mat_prop, V, output_choice, EF, K):
...
def mass_velocities_k (j, aero_coef, history, rho_air, geom_prop, mat_prop, V, output_choice, EF):
...
def plot_history_m (k, history,case): 
        
    plt.figure(1)
    plt.plot(k, history['m'], linestyle='-', marker='p', color='b', linewidth=1, label='Mass')
    ...
    plt.figure(2)
    plt.plot(k, history['Vf_OL'], linestyle='-', marker='p', color='g', linewidth=1, label='Open Loop (OL)')
    plt.plot(k, history['Vf_CL'], linestyle='-', marker='p', color='r', linewidth=1, label='Closed-Loop (CL)')
    plt.plot(k, history['Vf_OL'][0]*np.ones(len(k)), 'k:', linewidth=1, label='Minimum CL velocity allowed')
    plt.xlabel('Iterations')
    plt.title('Flutter Velocities')
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('Results/'+str(case)+'/Vflutter.png')
    
    plt.figure(3)
    plt.plot(k, history['f_obj'], linestyle='--', marker='p', color='m', linewidth=1, label='Objective Function')
    plt.xlabel('Iterations')
    ...
    plt.xlabel('Iterations')
    plt.title('Cross-section Shell Thickness')
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('Results/'+str(case)+'/th1.png')
    ...
    plt.figure(14)
    plt.plot(k, history['constraint'], linestyle='-', marker='p', color='k', linewidth=1, label='Constraint')
    plt.xlabel('Iterations')
    plt.ylabel('1-$Vf_{CL}$/$Vf_{init}$')
    plt.title('Constraint $Vf_{CL}$>$Vf_{init}$')
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('Results/'+str(case)+'/constraint.png')    
...
def plot_history_k (k, history,case): 
    ...
    plt.figure(9)
    plt.plot(k, history['Kh'], linestyle='-', marker='p', color='b', linewidth=1, label='Kh')
    plt.xlabel('Iterations')
    plt.title('Control Gain')
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('Results/'+str(case)+'/Kh.png')
    ...
    plt.figure(11)
    plt.plot(k, history['Khd'], linestyle='--', marker='p', color='y', linewidth=1, label='Khd')
    plt.xlabel('Iterations')
    plt.title('Control Gain')
    plt.legend(loc='best')
    plt.grid(True)
    plt.savefig('Results/'+str(case)+'/Khd.png')
    ...
    plt.figure(13)
    plt.plot(k, history['constraint'], linestyle='-', marker='p', color='k', linewidth=1)
    plt.xlabel('Iterations')
    plt.ylabel('1-$Vf_{CL}$/$Vf_{init}$')
    plt.title('Constraint $Vf_{CL}$>$Vf_{init}$')
    plt.grid(True)
